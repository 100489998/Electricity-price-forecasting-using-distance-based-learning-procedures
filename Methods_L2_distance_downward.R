###############################################################################
# Training and testing the methods (L2 distance and downward secondary market)
###############################################################################
...
save(R2DA, file = "R2DA_L2_godown.RData")
...
save(R2, file = "R2_L2_godown.RData")
...
for (j in 1:TestSize){
  ...
  DailyDistanceMax = array(0, c(d0-1,1)) # vector where to save the maximum of the distances between curves of the same hour d0-1 and the previous days
...
for (lag in 24:1){
  ...
  for(j in 1:TestSize){
    ...
    step_fun_real = approxfun(offers_19_21_without_NA[[i]]/sd_offers_19_21, 
                               prices_19_21_without_NA[[i]]/sd_prices_19_21, method = "constant",
                               f = 1, rule = 2, ties = base::max) # function of real the supply curve 
    prices_real = step_fun_real(Xvals) # Obtain real prices
...
rmse_godown_L2 = c(rmse_bmax, rmse_bsum, rmse_DA, rmse_rf)
save(rmse_godown_L2, file = "rmse_godown_L2.RData")
mae_godown_L2 = c(mae_bmax, mae_bsum, mae_DA, mae_rf)
save(mae_godown_L2, file = "mae_godown_L2.RData")
