...
# Read the MercadoSecundarioBajar data
...
for (i in 1:NHours_19_21){
  ...
  Xvals_100[i,] = seq(Xmin, Xmax, length.out = N_100) # grid of the cumulative offers
  ...
  prices_19_21_100[i,] <- step_fun(Xvals_100[i,]) # save the prices corresponding to the xticks
}
...
save(D, file = "D_19_21_godown_pf.RData")
...
save(D, file = "D_19_21_godown_pL2.RData")


# Compute distance matrix using the L2 distance
...
Xmin = 0 # minimum xtick
Xmax = min(MaxOffer) # Minimum maximum cumulative offer
...
for (i in 1:NHours_19_21){
  step_fun <- approxfun(offers_19_21_without_NA[[i]], prices_19_21_without_NA[[i]], method = "constant",
                        f = 1, rule = 2, ties = base::max) # define the supply curve
  prices_19_21_L2[i,] <- step_fun(Xvals) # save the prices corresponding to each xtick and curve
}


D = sqrt(Xvals[2])*as.matrix(distances(prices_19_21_L2))

save(D, file = "D_19_21_godown_L2.RData")
