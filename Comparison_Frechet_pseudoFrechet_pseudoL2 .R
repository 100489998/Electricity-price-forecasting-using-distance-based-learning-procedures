########################################################################
# Comparison between Fréchet and pseudo-Fréchet and pseudo-L2 distances
########################################################################
...
for (i in 1:NHours_19_20){
  ...
  step_fun = approxfun(offers_19_20_without_NA[[i]], prices_19_20_without_NA[[i]], method = "constant",
                        f = 1, rule = 2, ties = max) # define the function of the supply curve
...
for (i in 1:NHours_19_20){
  ...
  step_fun = approxfun(offers_19_20_without_NA[[i]], prices_19_20_without_NA[[i]], method = "constant",
                        f = 1, rule = 2, ties = max) # compute an step function of each curve
...
df_comp_pf = data.frame(
  "Fréchet distance"= D_distFrechet,
  "Pseudo-Fréchet distance with 100 points" = D_19_20_pf_100,
  "Pseudo-Fréchet distance with 200 points" = D_19_20_pf_200
)

pairs(df_comp_pf, labels = c("Fréchet distance", "Pseudo-Fréchet distance (100 offers)", "Pseudo-Fréchet distance (200 offers)"), cex.labels=2.1, cex.axis = 2.1)
...
pairs(df_comp_pL2, labels = c("Fréchet distance", "Pseudo-L2 distance (100 offers)", "Pseudo-L2 distance (200 offers)"), cex.labels=2.1, cex.axis = 2.1)
