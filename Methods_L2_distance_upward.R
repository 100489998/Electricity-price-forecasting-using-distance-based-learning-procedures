###############################################################################
# Training and testing the methods (L2 distance and upward secondary market)
###############################################################################
...
# Load matrix
load("D_19_21_goup_L2.RData")

# Load offers and prices for price forecasting 
load("offers_19_21_without_NA_goup.Rdata")
load("prices_19_21_without_NA_goup.Rdata")

load("sd_offers_19_21_goup.RData")
load("sd_prices_19_21_goup.RData")

# Load requirements
RequerimientosSecundariaASubir = read_excel("RequerimientosSecundariaASubir.xlsx")
req_19_21 = subset(RequerimientosSecundariaASubir, substr(datetime, 1, 4) == c("2019")  | substr(datetime, 1, 4) == c("2020") | substr(datetime, 1, 4) == c("2021"))
...
RFmodelingDA = function(D, Indexes1, Indexes2, Indexes)
{
  ...
  # Compute distances d(C_t,C_s), d(C_{t-1},C_{s-1}), ..., d(C_{t-23}, C_{s-23})
  for (l in 1:24){
    Xtmp = D[Indexes1-l, Indexes2-l]
    X[,l] = c(Xtmp)
  }
  
  # Compute distances d(C_t,C_{s+24}), d(C_{t-1}, C_{s+23}), ..., d(C_{t-23}, C_{s+1})
  for (l in 1:24){
    Xtmp = D[Indexes1-l, Indexes2+24-l]
    X[,l+24] = c(Xtmp)
  }
  
  # Compute distances d(C_{t+24},C_{s+24}), d(C_{t+23}, C_{s+23}), ..., d(C_{t+1},C_{s+1})
...
RFmodeling = function(D, Indexes1, Indexes2, Indexes, Lag)
{
  ...
  # Compute distances d(C_{t}, C_{s+h}), d(C_{t-1}, C_{s+h}),..., d(C_{t-23}, C_{s+h})
  for (l in 1:24){
    Xtmp = D[Indexes1-l, Indexes2+24-Lag] # Lag en Indexes2
    X[, l+24] = c(Xtmp)
  }
  
  # Distances d(C_t+h, C_s+h)
...
save(R2DA, file = "R2DA_L2_goup.RData")
...
min = round(min(error_DA_means, error_rf_means, error_knn_max_means, error_knn_sum_means),2)
max = round(max(error_DA_means, error_rf_means, error_knn_max_means, error_knn_sum_means),2)
breaks = seq(min, max, by = 0.02)

# Create the plot 
ggplot(data, aes(x = Hour)) +
  geom_point(aes(y = error_rf_means, color = "RF(h) + 1-NN"), size = 4, shape = 16) +
  geom_point(aes(y = error_DA_means, color = "RF + 1-NN"), size = 4, shape = 16) +
  geom_point(aes(y = error_knn_max_means, color = "1-NN (max)"), size = 4, shape = 16) +
  geom_point(aes(y = error_knn_sum_means, color = "1-NN (sum)"), size = 4, shape = 16) +
  geom_hline(yintercept = mean(data$error_rf_means), linetype = "solid", color = "black") +
  geom_hline(yintercept = mean(data$error_DA_means), linetype = "solid", color = "red") +
  geom_hline(yintercept = mean(data$error_knn_max_means), linetype = "solid", color = "blue") +
  geom_hline(yintercept = mean(data$error_knn_sum_means), linetype = "solid", color = "green") +
...
for (h in 1:24){
  for(j in 1:TestSize){
    ...
    errors_res_DA[j,h]= (Xvals[2]-Xvals[1])*distances(rbind(prices_real, prices_pred))[1,2] # Distance between predicted and real curves
  }
  cat(h)
}
...
for (lag in 24:1){
  ...
  for(j in 1:TestSize){
    ...
    errors_res_rf[j,h]= (Xvals[2]-Xvals[1])*distances(rbind(prices_real, prices_pred))[1,2] # Distance between predicted and real curves
  }
  cat(lag)
}
...
for (h in 1:24){
  for(j in 1:TestSize){
    ...
    errors_res_bmax[j,h]= (Xvals[2]-Xvals[1])*distances(rbind(prices_real, prices_pred))[1,2] # Distance between predicted and real curves
  }
  cat(h)
}
...
for (h in 1:24){
  for(j in 1:TestSize){
    ...
    errors_res_bsum[j,h]= (Xvals[2]-Xvals[1])*distances(rbind(prices_real, prices_pred))[1,2] # Distance between predicted and real curve
  }
  cat(h)
}
...
rmse_goup_L2 = c(rmse_bmax, rmse_bsum, rmse_DA, rmse_rf)
save(rmse_goup_L2, file = "rmse_goup_L2.RData")
mae_goup_L2 = c(mae_bmax, mae_bsum, mae_DA, mae_rf)
save(mae_goup_L2, file = "mae_goup_L2.RData")
