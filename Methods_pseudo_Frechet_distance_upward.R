###########################################################################################
# Training and testing the methods (pseudo-Fréchet distance and upward secondary market)
###########################################################################################
...
save(R2DA, file = "R2DA_pf_goup.RData")
...
min = 1
max = 45
breaks = c(1,2.5,seq(5, max, by = 10))
...
for (h in 1:24){
  for(j in 1:TestSize){
    ...
    errors_res_DA[j,h]= max(sqrt((prices_real-prices_pred)^2)) # Distance between predicted and real curves
  }
  cat(h)
}
...
for (lag in 24:1){
  ...
  for(j in 1:TestSize){
    ...
    errors_res_rf[j,h]= max(sqrt((prices_real-prices_pred)^2)) # Distance between predicted and real curves
  }
  cat(lag)
}
...
for (h in 1:24){
  for(j in 1:TestSize){
    ...
    errors_res_bmax[j,h]= max(sqrt((prices_real-prices_pred)^2)) # Distance between predicted and real curves
  }
  cat(h)
}
...
for (h in 1:24){
  for(j in 1:TestSize){
    ...
    errors_res_bsum[j,h]= max(sqrt((prices_real-prices_pred)^2)) # Distance between predicted and real curve
  }
  cat(h)
}
...
rmse_goup_pf = c(rmse_bmax, rmse_bsum, rmse_DA, rmse_rf)
save(rmse_goup_pf, file = "rmse_goup_pf.RData")
mae_goup_pf = c(mae_bmax, mae_bsum, mae_DA, mae_rf)
save(mae_goup_pf, file = "mae_goup_pf.RData")
