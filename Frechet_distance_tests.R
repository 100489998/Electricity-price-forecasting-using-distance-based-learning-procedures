##################################
# Discrete Fréchet distance tests
##################################

# Required libraries
library(TSdist) # for discrete Frechet distance
library(longitudinalData) # for discrete Frechet distance
library(ggplot2)
library(dplyr)
...
#-------------------------------------------------------
# Calculate discrete Fréchet distance computation times
#-------------------------------------------------------
...
#---------------------------------------------------------------------------------------------------------------------------------
# Select a random sample of the prices/offers of each hour and day to reduce computational cost of the discrete Frechet distance
#---------------------------------------------------------------------------------------------------------------------------------
...
options(max.print=1) # to avoid prints that increase computational cost
frech_dist_random = numeric(n) # vector where to save the approximate discrete Frechet distances
...
# Compute the difference between the exact discrete Frechet distance and the approximate discrete Frechet distance (in percentage)
for(i in 1:n){
  first_curve = first_curve_indexes[i]
  second_curve = second_curve_indexes[i]
...
hist(diff_per_distFrechet_random, breaks = round(sqrt(n)), xlab = "Percentage of difference between exact and approximate discrete Fréchet distances", xlim = c(0,max(diff_per_distFrechet_random)), main ="")


#-------------------------------------------------------------------------------------------------------------------------------------
# Select a sample taking the offers corresponding to the highest prices to reduce computational cost of the discrete Frechet distance
#-------------------------------------------------------------------------------------------------------------------------------------
...
max_diff_per = numeric(length(per)) # vector where to save the maximum difference between exact and approximate discrete Frechet distances
...
frech_dist_distFrechet_jumps = matrix(0, nrow = n, ncol = length(per)) # matrix where to save the approximate discrete Frechet distances
frech_dist_exact_jumps = matrix(0, nrow = n, ncol = length(per)) # matrix where to save the exact discrete Fréchet distances
