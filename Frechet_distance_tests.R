...
# Add the pair (0,0) in offers and prices datasets
...
options(max.print=1) # to avoid prints that increase computational cost
frech_dist_random = numeric(n) # vector where to save the approximated Frechet distances
frech_dist_exact = numeric(n) # vector where to save the exact distances
diff_per_distFrechet_random = numeric(n) # vector where to save the difference between the approximated and exact distances


# Compute the Difference between the exact Frechet distance and the approximated Frechet distance (in percentage, dividing by the minimum of the two distances)  
for(i in 1:n){
  first_curve = first_curve_indexes[i]
  second_curve = second_curve_indexes[i]
...
# Compute the differences between consecutive points in all the curves
...
# Find the percentage of offers with highest prices which gives the minimum differences

per = seq(0, 1, 0.05) # percentages of offers with the highest prices
max_diff_per = numeric(length(per)) # vector where to save the maximum difference between exact and approximated Frechet distance
...
frech_dist_distFrechet_jumps = matrix(0, nrow = n, ncol = length(per)) # matrix where to save the approximated Frechet distances
frech_dist_exact_jumps = matrix(0, nrow = n, ncol = length(per)) # matrix where to save the exact Fréchet distances
diff_per_distFrechet_jumps = matrix(0, nrow = n, ncol = length(per)) # matrix where to save the difference between the approximated and exact distances
...
for (j in 1:length(per)){
  for (i in 1:NHours_19_20){
    ...
    # (1-per[j])*100% of the offers  will be equidistant points selected among the offers that do not have the largest prices
