...
offer.all_19_20 <- subset(offer.all, substr(date, 1, 4) == c("2019")  | substr(date, 1, 4) == c("2020"))
price.all_19_20 <- subset(price.all, substr(date, 1, 4) == c("2019")  | substr(date, 1, 4) == c("2020"))
...
#-----------------------------------------------------------------
# Represent the curves and requirements of the first day of 2019
#-----------------------------------------------------------------

# Plot the supply curves

# Obtain the maximum offer and price to use it in the limits of the plots
max_offer <- numeric()
max_price <- numeric()
...
p <- ggplot(data, aes(x = offers, y = prices, color = factor(curve))) +
  ...
  ggtitle("") +
  theme_bw() +
  theme(panel.grid = element_blank(),          # Quitar las barras grises de los plots
        strip.text = element_text(size = 18),   # Tamaño de los títulos de categoría
        axis.title = element_text(size = 18),  # Tamaño de los títulos de ejes
        axis.text = element_text(size = 18),  # Tamaño de los números de ejes
...
for (i in c(1, 15, 18, 24)) {
  f <- approxfun(offers_19_20_without_NA[[i]], prices_19_20_without_NA[[i]], method = "constant",
                 f = 1, rule = 2, ties = max) # function of the supply curve
  df <- data.frame(
...
for(i in 1:NHours_19_20){
  #f <- stepfun(offers_19_20_without_NA[[i]], c(prices_19_20_without_NA[[i]], Inf), right = TRUE)
  f <- approxfun(offers_19_20_without_NA[[i]], prices_19_20_without_NA[[i]], method = "constant",
                 f = 1, rule = 2, ties = max)
  for(j in 1:length(offers_acf)){
    prices_vals[i,j+1] = f(offers_acf[j])
